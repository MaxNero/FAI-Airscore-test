...
pg_preset = FormulaPreset(
    ...
    # Lead Factor: factor for Leadout Points calculation formula [0 - 0.26, default 0.26]
    ...
    line_calc=Preset(value='optimised', visible=False),
    ...
    nominal_launch=Preset(value=0.96, visible=True, editable=False),
    # Nominal Goal: number of pilots that we assume should reach goal
    nominal_goal=Preset(value=0.3, visible=True, editable=False),
    ...
    score_back_time=Preset(value=300, visible=True, editable=False, comment='fixed: 5 mins'),
    # Jump the Gun: 1 or 0
    ...
    glide_bonus=Preset(value=2.5, visible=True, editable=False, comment='fixed: 2.5'),
    # Waypoint radius tolerance for validation: FLOAT default is 0.1%
    tolerance=Preset(value=0.001, visible=True, editable=False, comment='fixed: 0.1%'),
...
hg_preset = FormulaPreset(
    ...
    # Lead Factor: factor for Leadout Points calculation formula [0 - 0.26, default 0.175]
    lead_factor=Preset(value=0.175, visible=True, editable=True),
    # Lead Coeff formula: classic, weighted, integrated
    ...
    # Nominal Distance: nominal distance to be used for time validity calculation
    nominal_dist=Preset(value=50000, visible=True, editable=True),
    # Nominal Time: nominal time to be used for time validity calculation
    nominal_time=Preset(value=5400, visible=True, editable=True),
    # Nominal Launch: number of pilots that can DNF and still get full validity
    nominal_launch=Preset(value=0.96, visible=True, editable=False),
    ...
    # Score back time for Stopped Tasks (minutes)
    score_back_time=Preset(value=900, visible=True, editable=False, comment='fixed: 15 mins'),
...
def points_weight(task):
    ...
    ''' Distance Weight'''
    """ GAP2025: 0.9 - 1.665* goalRatio + 1.713*GolalRatio^2 - 0.587*goalRatio^3"""
    if task.formula.formula_distance != 'off':
        task.dist_weight = 0.9 - 1.665 * goal_ratio + 1.713 * goal_ratio ** 2 - 0.587 * goal_ratio ** 3

    """
    LeadingTimeRatio defines the ratio between leading and time weight, with a value between 0 and 26%. 
    ...
    Default value: PG 26%, HG 17.5%.
    """
    LeadingTimeRatio = task.formula.lead_factor

    if comp_class == 'HG':
        """
        LeadWeight:         (1 - DistWeight) * LeadingTimeRatio
        ArrWeight:          (1 - DistWeight) * 12.5%
        TimeWeight:         1 − DistWeight − LeadWeight − ArrWeight
        """

        ''' Arrival Weight'''
        if task.formula.formula_arrival != 'off':
            task.arr_weight = (1 - task.dist_weight) * 0.125
...
def pilot_speed(task, res):
    ...
    """
    if res.ESS_time is None or not task.avail_time_points:
        return 0
